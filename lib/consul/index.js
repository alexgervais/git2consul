...
var create_key_name = function(branch, file, ref) {
  ...
  if (branch.include_file_name || file.endsWith(".ref")) {
    key_parts.push(file);
  } else {
    var path_without_file_name = file.substr(0, file.lastIndexOf('/'));
    if (path_without_file_name) {
      key_parts.push(path_without_file_name);
    }
  }
...
var file_deleted = function(branch, file, cb) {
  // Dont delete subtree if file name is not included in namespace
