...
describe('Expand keys with include_file_name disabled', function() {
  ...
  beforeEach(function(done) {
    ...
    // Each of these tests needs a working repo instance, so create it here and expose it to the suite
    // namespace.  These are all tests of expand_keys mode, so set that here.
    git_utils.initRepo(_.extend(git_utils.createRepoConfig(), {'expand_keys': true, 'include_file_name': false}), function(err, repo) {
      if (err) return done(err);
  ...
  it ('should handle additions of new JSON files', function(done) {
    ...
    git_utils.addFileToGitRepo(sample_key, sample_value, "Add a file.", function(err) {
      ...
      branch.handleRefChange(0, function(err) {
        if (err) return done(err);
        // At this point, the repo should have populated consul with our sample_key
        consul_utils.validateValue('test_repo/master/first_level/second_level', 'is_all_we_need', function(err, value) {
          if (err) return done(err);
          done();
        });
      });
  ...
  it ('should handle changing JSON files', function(done) {
    ...
    git_utils.addFileToGitRepo(sample_key, sample_value, "Add a file.", function(err) {
      ...
      branch.handleRefChange(0, function(err) {
        if (err) return done(err);
        // At this point, the repo should have populated consul with our sample_key
        consul_utils.validateValue('test_repo/master/first_level', 'is_all_we_need', function(err, value) {
          if (err) return done(err);
          ...
          git_utils.addFileToGitRepo(sample_key, '{ "first_level" : "is super different" }', "Change a file.", function(err) {
            ...
            branch.handleRefChange(0, function(err) {
              ...
              // At this point, the repo should have populated consul with our sample_key
              consul_utils.validateValue('test_repo/master/first_level', 'is super different', function(err, value) {
                if (err) return done(err);
  ...
  it ('should handle busted JSON files', function(done) {
    ...
    git_utils.addFileToGitRepo(sample_key, sample_value, "Add a file.", function(err) {
      ...
      branch.handleRefChange(0, function(err) {
        if (err) return done(err);
        // At this point, the repo should have populated consul with our sample_key
        consul_utils.validateValue('test_repo/master', sample_value, function(err, value) {
          if (err) return done(err);
          ...
          git_utils.addFileToGitRepo(sample_key, '{ "not_busted" : "json" }', "Change a file.", function(err) {
            ...
            branch.handleRefChange(0, function(err) {
              ...
              // At this point, the repo should have populated consul with our sample_key
              consul_utils.validateValue('test_repo/master/not_busted', 'json', function(err, value) {
                if (err) return done(err);
  ...
  it ('should handle JSON files with special characters', function(done) {
    ...
    git_utils.addFileToGitRepo(sample_key, JSON.stringify(sample_value), "Add a file.", function(err) {
      ...
      branch.handleRefChange(0, function(err) {
        if (err) return done(err);
        // At this point, the repo should have populated consul with our sample_key
        consul_utils.validateValue('test_repo/master/fuzzy/second%20level', sample_value['fuzzy']['second level'], function(err, value) {
          // At this point, the repo should have populated consul with our sample_key
          consul_utils.validateValue('test_repo/master/fuzzy/second%2Flevel/ok%3F', sample_value['fuzzy']['second/level']['ok?'], function(err, value) {
            if (err) return done(err);
            done();
          });
        });
  ...
  it ('should handle additions of new properties files', function(done) {
    ...
    git_utils.addFileToGitRepo(sample_key, sample_value, "Add a file.", function(err) {
      ...
      branch.handleRefChange(0, function(err) {
        if (err) return done(err);
        // At this point, the repo should have populated consul with our sample_key
        consul_utils.validateValue('test_repo/master/foo', 'bar', function(err, value) {
          if (err) return done(err);
          done();
        });
      });
        ...
        consul_utils.validateValue('test_repo/master/foo', 'bar', function(err, value) {
          ...
          git_utils.addFileToGitRepo(sample_key, 'foo=different_bar', "Change a file.", function(err) {
            ...
            branch.handleRefChange(0, function(err) {
              ...
              // At this point, the repo should have populated consul with our sample_key
              consul_utils.validateValue('test_repo/master/foo', 'different_bar', function(err, value) {
                if (err) return done(err);
          ...
          branch.handleRefChange(0, function(err) {
            ...
            git_utils.addFileToGitRepo(property_key, property_value, "Add another file.", function(err) {
              ...
              branch.handleRefChange(0, function(err) {
                ...
                // At this point, the repo should have populated consul with our json_key
                consul_utils.validateValue('test_repo/master/happy', 'json', function(err, value) {
                  if (err) return done(err);

                  // At this point, the repo should have populated consul with our property_key
                  consul_utils.validateValue('test_repo/master/foo', 'bar', function(err, value) {
                    if (err) return done(err);
