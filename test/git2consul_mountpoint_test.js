...
describe('KV mountpoint', function() {
  ...
  it ('custom mountpoints should work with include_file_name disabled', function(done) {
    ...
    git_commands.init(git_utils.TEST_REMOTE_REPO, function(err) {
      ...
      git_utils.addFileToGitRepo("readme.md", "Test file mountpointed KV", "Test commit.", function(err) {
        ...
        repo.init(function(err) {
          if (err) return done(err);
          consul_utils.validateValue('nested/enough/for/my/purposes/test_repo/master', "Test file mountpointed KV", function(err, value) {
            if (err) return done(err);
            done();
          });
        });
